	...
	public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
	{
		if (!PropertyToCheck.IsNullOrEmpty())
		{
			...
			if (conditionProperty != null)
			{
                ...
                //    string compareStringValue = CompareValue[0]?.ToString().ToUpper() ?? "NULL";
                ...
                //} else if (CompareValue.Length > 1) {
                    string[] compareStringValue = new string[CompareValue.Length];
                    for (int i = 0; i < CompareValue.Length; i++) {
                        compareStringValue[i] = CompareValue[i]?.ToString().ToUpper() ?? "NULL";
                        if (isBoolMatch && compareStringValue[i] == "FALSE") isBoolMatch = false;
                    }

                    string conditionPropertyStringValue = conditionProperty.AsStringValue().ToUpper();
                    bool[] objectMatch = new bool[compareStringValue.Length];
                    for (int i = 0; i < objectMatch.Length; i++) {
                        objectMatch[i] = (compareStringValue[i] == conditionPropertyStringValue);
                    }

                    bool tmp2 = false;
                    for (int i = 0; i < objectMatch.Length; i++) {
                        if (objectMatch[i] == true) tmp2 = true;
                    }
